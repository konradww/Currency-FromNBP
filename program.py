...
def currency():
    dict = {}
    if soup.find_all('td')[36].get_text() == '1 EUR':
        dict[soup.find_all('td')[37].get_text()] = (soup.find_all('td')[37].get_text()).replace(",", ".")
    if soup.find_all('td')[38].get_text() == '1 USD':
        dict[soup.find_all('td')[39].get_text()] = (soup.find_all('td')[39].get_text()).replace(",", ".")
    if soup.find_all('td')[40].get_text() == '1 CHF':
        dict[soup.find_all('td')[41].get_text()] = (soup.find_all('td')[41].get_text()).replace(",", ".")
    if soup.find_all('td')[42].get_text() == '1 GBP':
        dict[soup.find_all('td')[43].get_text()] = (soup.find_all('td')[43].get_text()).replace(",", ".")
    return dict
...
for k, v in currency().items():
    ...
    question = ("BEGIN TRAN "
                ...
                "COMMIT"
                ) % (k, data, k, ast.literal_eval(v), data)
